...
class WebLinkProcessorSpec extends Specification {
    ...
    def "processor uses configured lexer instead of default"() {
        ...
        def validationResult = new WebLinkValidator.ValidationResult(List.of(), new WebLinkValidator.IssueReport(List.of()))
    ...
    def "aggregates issues from multiple validators and uses last validator's weblinks"() {
        ...
        def issue1 = WebLinkValidator.Issue.error("first")
        def issue2 = WebLinkValidator.Issue.warning("second")

        def result1 = new WebLinkValidator.ValidationResult(List.of(link1), new WebLinkValidator.IssueReport(List.of(issue1)))
        def result2 = new WebLinkValidator.ValidationResult(List.of(link2), new WebLinkValidator.IssueReport(List.of(issue2)))
    ...
    def "lexer exception is propagated and prevents parser and validators from running"() {
        ...
        then:
        1 * lexer.lex("<bad") >> { throw new WebLinkLexer.LexingException("boom") }
        0 * parser._
        0 * validator._

        and:
        thrown(WebLinkLexer.LexingException)
    }
    ...
    def "external mutation of issue list from validator does not break aggregated result"() {
        ...
        def mutableIssues = new ArrayList<WebLinkValidator.Issue>()
        mutableIssues.add(WebLinkValidator.Issue.error("original"))

        def validationResult = new WebLinkValidator.ValidationResult(
                List.of(dummyWebLink("l1")),
                new WebLinkValidator.IssueReport(mutableIssues)
        )
    ...
    def "external mutation of weblink list from validator does not alter processor result"() {
        ...
        def validationResult = new WebLinkValidator.ValidationResult(
                mutableWebLinks,
                new WebLinkValidator.IssueReport(List.of())
        )
